...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.cash < self.price:
            return 'You must deposit ${} more.'.format(self.price - self.cash)
        self.stock -= 1
        self.cash -= self.price
        if self.cash:
            result = 'Here is your {0} and ${1} change.'.format(self.item, self.cash)
            self.cash = 0
            return result
        else:
            return 'Here is your {}.'.format(self.item)

    def deposit(self, acount):
        if self.stock == 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(acount)
