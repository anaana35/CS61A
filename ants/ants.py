...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=float('inf')):
        ...
        for _ in range(0, min_range):
            nearest_place = nearest_place.entrance
        nearest_bee = None
        while not nearest_bee and nearest_place != hive and min_range < max_range:
            min_range += 1
            nearest_bee = random_or_none(nearest_place.bees)
            nearest_place = nearest_place.entrance
        return nearest_bee
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while back_place:
            ant = back_place.ant
            if ant and back_place.ant not in self.double_ants:
                self.double_ants.append(back_place.ant)
                back_place.ant.damage *= 2

            if isinstance(ant, BodyguardAnt) and ant.ant and ant.ant not in self.double_ants:
